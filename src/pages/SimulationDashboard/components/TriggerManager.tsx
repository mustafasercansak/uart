...
const ACTION_LABELS: Record<TriggerAction, string> = {
  'stop_simulation': 'Simülasyonu Durdur',
  'start_recording': 'Kaydı Başlat',
  'log_warning': 'Uyarı Logla',
  'inject_error': 'Hata Enjekte Et',
  'set_field': 'Alan Değerini Değiştir'
};
    ...
    <div className="flex flex-col h-full bg-gray-950 border-l border-gray-800 w-80 shrink-0 shadow-2xl overflow-hidden font-mono">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {triggers.length === 0 && !isAdding && (
          <div className="text-center py-10">
            ...
            <p className="text-[10px] text-gray-600 italic">Henüz aktif bir kural yok...</p>
          </div>
        )}
        ...
        {isAdding && (
          <div className="bg-gray-900/50 p-3 rounded-lg border border-emerald-500/30 space-y-3 animate-in fade-in slide-in-from-top-2">
            <div>
              <label className="text-[9px] text-gray-500 uppercase block mb-1">Kural Adı</label>
              <input 
            ...
            <div>
              <label className="text-[9px] text-gray-500 uppercase block mb-1">Koşul (JS)</label>
              <input 
                ...
                placeholder="Örn: BPM > 100"
              />
            </div>
            <div>
              <label className="text-[9px] text-gray-500 uppercase block mb-1">Aksiyon</label>
              <select 
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-800/50">
               ...
               {trigger.lastTriggeredAt && (
                 <span className="ml-auto text-[8px] text-emerald-500/50 animate-pulse">AKTİF</span>
               )}
            </div>
          </div>
        ))}
      ...
      <div className="p-3 bg-gray-900/50 border-t border-gray-800">
         <div className="flex items-center gap-2 text-rose-500/70">
            ...
            <span className="text-[9px] uppercase font-bold tracking-tight">Kritik Sensör İzleme Aktif</span>
         </div>
      </div>
    </div>
  );
