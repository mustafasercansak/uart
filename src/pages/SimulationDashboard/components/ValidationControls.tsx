        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                <Target size={12} /> {t('validation.testName')}
              </label>
              <input 
            ...
            <div className="space-y-4">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                <Cpu size={12} /> {t('validation.deviceId')}
              </label>
              <input 
            ...
            <div className="space-y-4 col-span-2">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                <User size={12} /> {t('validation.operator')}
              </label>
              <input 
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">{t('validation.criteria')}</h3>
              <button 
                onClick={addTarget}
              ...
              >
                <Plus size={12} /> {t('validation.addCriteria')}
              </button>
            </div>
            ...
            <div className="space-y-3">
              {targets.map((target, idx) => (
                <div key={target.id} className="flex gap-3 bg-black/40 border border-white/5 p-4 rounded-2xl items-end group animate-in slide-in-from-right-4 duration-300" style={{ animationDelay: `${idx * 50}ms` }}>
                  <div className="flex-1 space-y-2">
                    <label className="text-[9px] font-bold text-gray-600 uppercase">{t('validation.fieldName')}</label>
                    <select 
                  ...
                  <div className="w-16 space-y-2">
                    <label className="text-[9px] font-bold text-gray-600 uppercase">{t('validation.unit')}</label>
                    <input 
                      value={target.unit}
              ...
              {targets.length === 0 && (
                <div className="text-center py-12 border-2 border-dashed border-white/5 rounded-3xl">
                  <p className="text-xs text-gray-600 font-bold uppercase tracking-widest">{t('validation.noCriteria')}</p>
                </div>
              )}
            </div>
          </div>
