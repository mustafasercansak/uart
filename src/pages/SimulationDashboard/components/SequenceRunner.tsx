...
const SequenceRunner: React.FC = () => {
  ...
  useEffect(() => {
    if (state.activeSequenceId) {
      ...
      if (seq) {
       ...
       setSteps(first.steps as SequenceStep[]);
       setSequenceName(first.name);
       setActiveId(first.id);
    } else if (steps.length === 0 && !activeId) {
  ...
  const saveSequence = () => {
    ...
    const newSequence = {
        ...
        createdAt: activeId ? state.sequences.find(s => s.id === activeId)?.createdAt : new Date().toISOString()
    };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-gray-950/40 p-4">
      ...
      <div className="mt-4 p-3 bg-gray-900/60 rounded-xl border border-gray-800 flex items-center justify-between font-mono">
        <span className="text-[9px] text-gray-500 uppercase">Status: <span className={isRunning ? 'text-blue-400 animate-pulse' : 'text-gray-400'}>{isRunning ? 'EXECUTING' : 'READY'}</span></span>
        <div className="flex gap-4">
           <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> <span className="text-[9px] text-gray-600">PASS: {steps.filter(s => s.status === 'success').length}</span></div>
           <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-rose-500" /> <span className="text-[9px] text-gray-600">FAIL: {steps.filter(s => s.status === 'fail').length}</span></div>
        </div>
      </div>
    </div>
  );
